...
export async function addCommand(components, opts) {
    const cwd = opts.cwd ?? process.cwd();
    // ── 1. Pre-flight: check shadcn setup ─────────────────────────
    if (!checkShadcnSetup(cwd)) {
        log.warn('No shadcn/ui setup detected (missing components.json).');
    ...
    if (entries.length > 1) {
        console.log();
        log.success(`Done! ${entries.length} component(s) installed.`);
    }
}

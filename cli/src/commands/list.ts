...
export function listCommand(opts: { cwd?: string }): void {
  ...
  console.log(
    `  ${pc.dim('Status')}  ${pc.dim('Name'.padEnd(nameWidth))} ${pc.dim('Description')}`
  )
  console.log(
    `  ${pc.dim('──────')}  ${pc.dim('─'.repeat(nameWidth))} ${pc.dim('─'.repeat(descWidth))}`
  )
  ...
  for (const entry of registry) {
    ...
    const desc = pc.dim(
      entry.description.length > descWidth

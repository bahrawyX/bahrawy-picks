...
import { type RegistryEntry, GITHUB_RAW_BASE, sharedFiles } from './registry.js'
...
function componentsDir(cwd: string): string {
  return path.join(cwd, 'components', 'bahrawy')
}

/** Check if shadcn is initialised (components.json or ui dir exists) */
export function checkShadcnSetup(cwd: string): boolean {
  return (
    fs.existsSync(path.join(cwd, 'components.json')) ||
    fs.existsSync(path.join(cwd, 'components', 'ui'))
  )
}

/** Check if a component file already exists locally */
export function isInstalled(name: string, files: string[], cwd: string): boolean {
...
async function fetchSharedFile(srcPath: string): Promise<string> {
...
export async function installComponent(
  ...
  if (!opts.force && isInstalled(entry.name, entry.files, cwd)) {
    log.warn(
      `${entry.name} already exists. Use ${pc.bold('--force')} to overwrite.`
    )
    return
  }
  ...
  if (allDevDeps.length > 0) {
    ...
    try {
      ...
      execSync(cmd, { cwd, stdio: 'pipe' })
      devSpinner.succeed(
        `Installed dev: ${pc.dim(allDevDeps.join(', '))}`
      )
    } catch {
      devSpinner.fail('Failed to install dev dependencies')
      log.dim(
        `  Run manually: ${installCmd(pm)} ${devFlag(pm)} ${allDevDeps.join(' ')}`
      )
    }
  }
  ...
  if (entry.shadcnComponents.length > 0) {
    ...
    try {
      ...
      execSync(cmd, { cwd, stdio: 'pipe' })
      shadcnSpinner.succeed(
        `Added shadcn: ${pc.dim(entry.shadcnComponents.join(', '))}`
      )
    } catch {
      shadcnSpinner.fail('Failed to add shadcn components')
      log.dim(
        `  Run manually: npx shadcn@latest add ${entry.shadcnComponents.join(' ')}`
      )
    }
  }

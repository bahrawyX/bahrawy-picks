          ...
          <table className="relative w-full min-w-[640px] border-collapse text-left">
            ...
            <thead>
              <tr className="border-b border-white/10">
                <th className="w-1/3 p-5 text-left text-xs font-medium uppercase tracking-[0.14em] text-white/40">
                  Features
                </th>
                {plans.map((plan) => (
                  <th key={plan.id} className="p-5 text-left align-top">
...
function CompareRow({ feature, plans, accentColor, stagger }: CompareRowProps) {
  return (
    <motion.tr
    ...
    >
      ...
      {plans.map((plan, i) => {
        ...
        return (
          <td key={plan.id} className="p-4 text-sm text-white/70">
            {typeof v === 'boolean' ? (
              v ? (
                <Check
                  className="h-4 w-4"

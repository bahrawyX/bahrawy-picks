...
export function AppShell({
  ...
  return (
    <div
      className={cn(
        'relative isolate flex h-full w-full overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0a0a0c]',
        className,
      )}
    >
      {/* Desktop sidebar */}
      <aside
        ...
        className="relative hidden h-full shrink-0 flex-col border-r border-white/[0.06] backdrop-blur-2xl md:flex"
      >
        <div
          aria-hidden
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header
          className="relative z-10 flex h-[52px] shrink-0 items-center gap-3 border-b border-white/[0.06] px-4 backdrop-blur-xl md:px-6"
          style={{
            background:
        ...
        >
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-md text-white/65 transition-colors hover:bg-white/[0.06] hover:text-white md:hidden"
          >
    ...
    <div className="relative flex h-full min-h-0 flex-col">
      ...
      <div className="flex h-[52px] shrink-0 items-center justify-between gap-2 border-b border-white/[0.06] px-4">
        ...
        {onCloseMobile && (
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-md text-white/55 transition-colors hover:bg-white/[0.06] hover:text-white"
          >
      ...
      <nav className="flex-1 space-y-6 overflow-y-auto px-3 py-5">
        {sidebar.map((section, i) => (
          <div key={i}>
            {section.label && (
              <p className="px-3 pb-2 text-[10.5px] font-medium uppercase tracking-[0.16em] text-white/40">
                {section.label}
              </p>
            )}
    ...
    <>
      {item.active && (
        <motion.span
          ...
          aria-hidden
          className="absolute inset-0 rounded-lg border border-white/[0.1] bg-white/[0.08]"
          transition={SPRING}
        />
      )}
      {item.icon && (
